...
use eip712_stand_alone as eip712;
...
fn tx_to_digest(
    ...
    for (_key, input_group_idxs) in input_groups_idxs {
        ...
        let (digest, typed_data_hash, chain_id, _) =
...
fn to_typed_action(parser: &WitnessesParser) -> Result<Value, Box<dyn ScriptError>> {
    let action = String::from_utf8(parser.action.clone())
        .map_err(|_| ErrorCode::EIP712SerializationError)?;
                ...
                match type_opt {
                    Some(type_script) => {
                        ...
                        if util::is_reader_eq(
                            ...
                            type_id_table_reader.balance_cell(),
                        ) {
                            i += 1;
                            continue;
                        }
...
fn to_semantic_account_witness(
    ...
    let (version, _, entity) =
        parser.verify_with_hash_and_get(expected_hash, data_type, index, source)?;
    let witness: Box<dyn AccountCellDataMixer> = match version {
        2 => Box::new(
            das_packed::AccountCellDataV2::from_slice(entity.as_reader().raw_data()).map_err(
                |_| {
        ...
        3 => Box::new(
            das_packed::AccountCellDataV3::from_slice(entity.as_reader().raw_data()).map_err(
                |_| {
        ...
        4 => Box::new(
            das_packed::AccountCellData::from_slice(entity.as_reader().raw_data()).map_err(
                |_| {
