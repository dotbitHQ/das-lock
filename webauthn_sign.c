...
int get_tx_digest_from_json(uint8_t * tx_digest, uint8_t* json_data, size_t json_len){
    ...
    if(memcmp(tx_digest_str, COMMON_PREFIX, COMMON_PREFIX_LENGTH) == 0){
        ...
        str2bin(tx_digest + COMMON_PREFIX_LENGTH, (unsigned char*)(tx_digest_str + COMMON_PREFIX_LENGTH), challenge_len - COMMON_PREFIX_LENGTH);
    }else {
...
//    }
    ...
    ret = memcmp(tx_digest, COMMON_PREFIX, COMMON_PREFIX_LENGTH) + memcmp(tx_digest + COMMON_PREFIX_LENGTH, message, HASH_SIZE);
    if(ret != 0){
...
__attribute__((visibility("default"))) int validate_device(
        ...
        uint8_t* msg,
        size_t msg_len,
        uint8_t* device_key_list,
        size_t device_key_list_len,
        uint8_t* data,
        size_t data_len
){
    ...
    if(device_key_list_payload_num > 10 || device_key_list_payload_num < 1){
        debug_print("device_key_list_payload_num is not correct");
        return ERROR_ARGUMENTS_LEN;
    }
