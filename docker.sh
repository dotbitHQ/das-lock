...
RUST_CONTRACTS=(dispatch eip712-lib)

function is_feature_available() {
  case $1 in
  testnet2 | testnet3 | mainnet) ;;
...
function build_all() {
  for contract in "${RUST_CONTRACTS[@]}"; do
    if [[ -d $contract ]]; then
      build $contract $1
    fi
  done
