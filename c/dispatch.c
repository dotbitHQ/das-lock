 ...
 */
int
get_data_hash_inner(uint8_t *output, size_t *output_len, uint8_t *temp, size_t temp_len, uint8_t *lock_args, int field,
                    bool is_owner) {
...
int
get_data_hash(uint8_t *output, size_t *output_len, uint8_t *temp, uint8_t *lock_args, bool is_owner, bool *in_inputs) {
    size_t temp_len = TEMP_SIZE;

    //get data.hash from cell_deps
    if (get_data_hash_inner(output, output_len, temp, temp_len, lock_args, CKB_SOURCE_INPUT, is_owner) ==
        ERROR_DEVICE_KEY_LIST_CELL_NOT_FOUND) {
        temp_len = TEMP_SIZE;
        *in_inputs = false;
...
int get_payload_from_cell(uint8_t *lock_args, uint8_t *temp, uint8_t pk_idx, uint8_t args_index) {
    ...
    //get data.hash from cell_deps
    ret = get_data_hash(data_hashs, &data_hashs_len, temp, lock_args, args_index == 0 ? true : false,
                        &device_key_list_cell_in_inputs);
    SIMPLE_ASSERT(0);
    ...
    for (size_t i = start_idx; i < end_idx; i++) {
        ...
        if (memcmp(temp, "das", 3) == 0) {
            if (memcmp(temp + 3, data_type_1, 4) == 0 && device_key_list_cell_in_inputs == true) {
                ...
                ret = get_payload_from_molecule_entity_data(lock_args, &lock_args_len, temp + 7, witness_len - 7,
                                                            pk_idx, data_hashs, data_hashs_len, OLD);
                if (ret != 0) {
                ...
                ret = get_payload_from_molecule_cell_data(lock_args, &lock_args_len, temp + 7, witness_len - 7, pk_idx,
                                                          data_hashs, data_hashs_len);
                if (ret != 0) {
