...
int verify_signature_secp256r1(uint8_t *sig, uint8_t *msg, size_t msg_len, uint8_t *pk) {
    ...
    debug_print_int("Verify the signature, ret", ret);
...
int get_tx_digest_from_json(uint8_t *tx_digest, uint8_t *json_data, size_t json_len) {
    ...
    //The protocol length here is fixed at 86 bytes, with TEMP_SIZE_SMALL being a bit of a margin.
    size_t challenge_len = TEMP_SIZE_SMALL;
    char challenge_str[TEMP_SIZE_SMALL];
    ...
    char tx_digest_str[TEMP_SIZE_SMALL];//The tx_digest has a fixed length of 64 characters.
    size_t tx_digest_len = TEMP_SIZE_SMALL;
    ret = decode_base64url_to_string(tx_digest_str, &tx_digest_len, challenge_str, &challenge_len);
    debug_print_int("decode base64url, ret", ret);
    SIMPLE_ASSERT(0);

    //convert from string to bytes
    //COMMON_PREFIX(11) is the length of "From .bit "
    ...
    if (memcmp(tx_digest_str, COMMON_PREFIX, COMMON_PREFIX_LENGTH) == 0) {
        ...
        str2bin(tx_digest + COMMON_PREFIX_LENGTH, (unsigned char *) (tx_digest_str + COMMON_PREFIX_LENGTH),
                tx_digest_len - COMMON_PREFIX_LENGTH);
    } else {
...
__attribute__((visibility("default"))) int validate(int type, uint8_t *message, uint8_t *lock_bytes, uint8_t *lock_args) {
    ...
    if (pk_idx != 255) {
        if (pk_idx > 9) {
            debug_print_int("public key index out of bounds, pk_idx", pk_idx);
